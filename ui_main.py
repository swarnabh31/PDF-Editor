...
from PySide6.QtGui import (
    QPixmap, QColor, QFont, QPen, QBrush, QImage, QCursor, QPainter, QFontDatabase
)
...
# =========================================================================
# SIGNATURE DIALOG (Fixed Transparency)
# =========================================================================
class SignatureCanvas(QWidget):
    ...
    def mouseMoveEvent(self, event):
        if (event.buttons() & Qt.MouseButton.LeftButton) and self.scribbling:
            ...
            painter.setPen(QPen(self.myPenColor, self.myPenWidth, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap,
    ...
    def choose_bg_color(self):
        ...
        if ok:
            if choice == "Transparent":
                self.canvas.set_bg_color(QColor(0, 0, 0, 0))
            elif choice == "White":
                self.canvas.set_bg_color(QColor(255, 255, 255))
            elif choice == "Yellow":
                self.canvas.set_bg_color(QColor(255, 255, 0))
...
class DraggableTextItem(QGraphicsRectItem):
    ...
    def __init__(self, overlay_data, parent=None):
        ...
        # Initialize font with style support
        font_family = overlay_data.get("qt_font_family", "Arial")
        font_size = overlay_data.get("font_size", 12)
        font = QFont(font_family, font_size)
        font.setBold(overlay_data.get("font_bold", False))
        font.setItalic(overlay_data.get("font_italic", False))
        font.setUnderline(overlay_data.get("font_underline", False))
    ...
    def update_style(self, new_data):
        ...
        if any(k in new_data for k in
               ["font_size", "font_name", "qt_font_family", "font_bold", "font_italic", "font_underline"]):
...
class DraggableImageItem(QGraphicsRectItem):
    ...
    def paint(self, painter, option, widget):
        ...
        if not self.pixmap.isNull():
            scaled = self.pixmap.scaled(self.rect().size().toSize(), Qt.AspectRatioMode.KeepAspectRatio,
...
class MainWindow(QMainWindow):
    ...
    def delete_current_page(self):
        ...
        reply = QMessageBox.question(self, "Confirm Delete",
                                     f"Delete Page {self.current_page + 1}? This cannot be undone.",
    ...
    def on_selection_changed(self):
        ...
        if boxes:
            ...
            # Hide style controls for images
            for widget in [self.prop_font_size, self.btn_text_color, self.btn_bg_color,
                           self.font_combo, self.btn_pick_font, self.btn_bold,
                           self.btn_italic, self.btn_underline]:
    ...
    def update_selected_box_font(self, font_display_name):
        ...
        new_data = {"qt_font_family": font_display_name}
        font_map = {"Arial": "helv", "Times New Roman": "tiro", "Courier New": "cour"}

        # If it's a standard font, update the internal font_name so pdf_engine knows what to use
        if font_display_name in font_map:
            new_data["font_name"] = font_map[font_display_name]
    ...
    def choose_color(self, target):
        ...
        if target == "bg":
            ...
            if not ok: return
            if choice == "Transparent":
                new_data = {"bg_color": "transparent"}
            elif choice == "White":
                new_data = {"bg_color": "#FFFFFF"}
            elif choice == "Yellow":
                new_data = {"bg_color": "#FFFF00"}
            else:
